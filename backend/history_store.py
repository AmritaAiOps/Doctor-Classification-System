...
"""
from backend.stages.daily_history import load_history, record_day
...
def get_month_records(year: int, month: int) -> list:
    ...
    days = sorted({int(d) for series in month_data.values() for d in series})
    return [
...
def has_day_one(year: int, month: int) -> bool:
    return any("1" in series for series in _month_data(year, month).values())
