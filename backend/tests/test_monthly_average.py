...
def test_snapshot_round_trip_and_has_day_one():
    _save(2026, 6, 1, 500.0)
    records = history_store.get_month_records(2026, 6)
    assert records == [{"day": 1, "values": {"Total Billing": 500.0}}]
    assert history_store.has_day_one(2026, 6)
    assert not history_store.has_day_one(2026, 5)


def test_month_without_day_one_never_averages():
